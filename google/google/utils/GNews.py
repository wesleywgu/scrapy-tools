...
def define_date(date):
    months = {'Jan': 1, 'Feb': 2, 'Mar': 3, 'Apr': 4, 'May': 5, 'Jun': 6, 'Jul': 7, 'Aug': 8, 'Sep': 9, 'Sept': 9,
              'Oct': 10, 'Nov': 11, 'Dec': 12, '01': 1, '02': 2, '03': 3, '04': 4, '05': 5, '06': 6, '07': 7, '08': 8,
...
class GNews:
    ...
    def __init__(self, period="", encode="utf-8"):
        self.__texts = []
        self.__links = []
        self.__results = []
        self.__totalcount = 0
    ...
    def page_at(self, page=1):
        ...
        try:
            if self.__period != "":
                self.url = "https://www.google.com/search?q={}&sca_esv=581520105&tbas=0&tbs=qdr:{},sbd:1&tbm=nws&sxsrf=AM9HkKkXb3sBmvO6GPX6Bk-OFf-AauWLOA:1699710999318&ei=F4hPZeqGE5vf2roPk-C5sA4&start={}&sa=N&ved=2ahUKEwiq7tbyjLyCAxWbr1YBHRNwDuY4ChDx0wN6BAgCEAI&biw=1680&bih=825&dpr=2&region=china".format(
                    self.__key, self.__period, (10 * (page - 1)))
            else:
                self.url = "https://www.google.co.jp/search?q={}&sca_esv=581520105&tbm=nws&sxsrf=AM9HkKlf_2GSpamrzPDRiFuZ8XAu2ANJMQ:1699711510136&ei=FopPZaTzB8nd2roPguakOA&sa=N&ved=2ahUKEwik1qDmjryCAxXJrlYBHQIzCQcQ8NMDegQIAxAW&biw=1680&bih=825&dpr=2&start={}&region=china".format(
                    self.__key, (10 * (page - 1)))
        ...
        try:
            ...
            for item in result:
                ...
                try:
                    tmp_desc = \
                        item.find_next_sibling('div').find('div').find_next_sibling('div').find('div').find('div').find(
                            'div').contents[0].replace('\n', '')
                except Exception:
                    tmp_desc = ''
                try:
                ...
                results.append(
                    {'title': tmp_text, 'media': tmp_media, 'date': tmp_date, 'datetime': define_date(tmp_date),
                     'desc': tmp_desc, 'link': tmp_link, 'img': tmp_img})
            self.response.close()
    ...
    def get_news(self, key="", deamplify=False):
        ...
        self.url = 'https://news.google.com/search?q={}'.format(key)
